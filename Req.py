...
class Req():
    ...
    async def __req(self, url: str, session: ClientSession, method: str, payload: str, semaphore: Semaphore, validate: bool = False) -> Union[dict[Any,Any],str]:
        """Private method. Returns the request as either a dict or string. Dictionary will always be the preferred return type
        ...
        :NOTE: If response status is (500) Internal Server Error, the request will be retried after 10 seconds"""
        try:
            async with semaphore:
                async with session.request(method=method, url=url, data=payload, timeout=self.__TIMEOUT) as response:
                    if response.status in range(200,299):
                        ...
                        try: r: dict[Any,Any] = await response.json()
                        except: r: str = await response.text()
                        if not r: r: dict[str,Any] = {"OK":f"{method}","HEADERS":response.headers}
                        return(r)
    ...
    async def make_requests(self, req_list: list[list[str]]) -> list[list[Union[dict[Any,Any],str]]]:
        ...
        - :Allowed methods: get, post, put, delete and patch. Payload must be a dictionary, it will be converted to a string
        ...
        resultsList: list = []
        partitions = self.returnPartionedList(req_list)
            ...
            for partition in partitions:
                ...
                for entry in partition:
                    if "get" in entry[0].lower():
                        ...
                        result = await self.__req(entry[1],session,entry[0],check_payload(entry),semaphore)
                        resultsList.append(result)
                if partitionTasks:
                    results: list[list[Union[dict[Any,Any],str]]] = await gather(*partitionTasks)
